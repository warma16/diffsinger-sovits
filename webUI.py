...
def upload_mix_append_file(files,sfiles):
    try:
        ...
        return file_paths,mix_model_output1(value=json.dumps(p,indent=2))
...
def updata_mix_info(files):
    try:
        if files is None :
            return mix_model_output1(value="")
        p = {file.name:100 for file in files}
        return mix_model_output1(value=json.dumps(p,indent=2))
    except Exception as e:
        if debug:
            traceback.print_exc()
        raise gr.Error(e)
...
def modelAnalysis(model_path,config_path,cluster_model_path,device,enhance,diff_model_path,diff_config_path,only_diffusion,use_spk_mix,local_model_enabled,local_model_selection):
    ...
    try:
        ...
        for i in spks:
            msg += i + " "
        return gr.Dropdown(choices = spks,value=spks[0],interactive=True), msg
    except Exception as e:
        if debug:
            traceback.print_exc()
        raise gr.Error(e)
...
def modelUnload():
    global model
    if model is None:
        return sid(choices = [],value=""),"没有模型需要卸载!"
    else:
        model.unload_model()
        model = None
        torch.cuda.empty_cache()
        return sid(choices = [],value=""),"模型卸载完毕!"
...
def local_model_refresh_fn():
    ...
    #gr.Dropdown(choices=symbols, interactive=True)
        ...
        with gr.TabItem("推理"):
            ...
            with gr.Row(variant="panel"):
                with gr.Column():
                    ...
                    f0_predictor = gr.Dropdown(label="选择F0预测器,可选择crepe,pm,dio,harvest,rmvpe,dspitch默认为pm(注意：crepe为原F0使用均值滤波器,dspitch是添加了diffsinger的提取器)", choices=["pm","dio","harvest","crepe","rmvpe","dspitch"], value="pm",interactive=True)
